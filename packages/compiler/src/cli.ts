...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === "--dataset-b") args.datasetB = argv[i + 1];
...
function usage(): string {
  return "Usage: thought-tagger-compile --spec <path> --dataset <path(.csv|.jsonl)> [--dataset-b <path(.csv|.jsonl)>] --out <dir> [--context-sidecar <path.jsonl>]";
}
...
compileStudy({
  ...
  datasetPathB: parsed.datasetB,
  outDir: parsed.out,
  contextSidecarPath: parsed.contextSidecar
})
  .then(() => {
