...
function assertValidCompareConfig(spec: StudySpec): void {
  if (spec.task_type !== "compare") {
    throw new Error("compare config is only supported when task_type is compare");
  }

  const mode = spec.compare?.shared_context_mode ?? "none";
  if (mode !== "none" && mode !== "inline_meta" && mode !== "sidecar_jsonl") {
    throw new Error(`Unsupported compare.shared_context_mode: ${mode}`);
  }

  if (mode === "inline_meta") {
    const field = spec.compare?.shared_context_field?.trim();
    if (!field) {
      throw new Error("compare.shared_context_field is required when shared_context_mode is inline_meta");
    }
  }

  if (mode === "sidecar_jsonl") {
    const sidecarPath = spec.compare?.shared_context_sidecar_path?.trim();
    if (!sidecarPath) {
      throw new Error("compare.shared_context_sidecar_path is required when shared_context_mode is sidecar_jsonl");
    }
  }
}
